...
rule all:
    input:
        filt=expand(
            "results/filter/{dataset}/{filter}_filtered.txt",
...
rule get_hmms:
    output:
        expand(
            "resources/hmm/bold.hmm{suff}", suff=["", ".h3f", ".h3i", ".h3m", ".h3p"]
        ),
    log:
        "logs/wget/wget.log",
    shadow:
...
rule filter:
    ...
    run:
        ...
        pandas_params = {
            ...
            "dtype": {
                "length": {"ASV": str, "length": int},
                "bitscore": {"ASV": str, "bitscore": float},
            },
        }
        df = pd.read_csv(
            input[0],
            sep=pandas_params["sep"][f],
            index_col=None,
            header=None,
            names=["ASV", f],
            usecols=pandas_params["usecols"][f],
            comment="#",
